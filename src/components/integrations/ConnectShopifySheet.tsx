...
interface ConnectShopifySheetProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    userId: string;
}

export function ConnectShopifySheet({ open, onOpenChange, userId }: ConnectShopifySheetProps) {
    const { addStore, updateStore } = useConnectedStores(userId);
    const [activeTab, setActiveTab] = useState<'add' | 'guide'>('add');
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent hideClose className="sm:max-w-md w-full flex flex-col h-full p-0 gap-0 bg-white overflow-hidden sm:border-l sm:shadow-2xl">
                <SheetHeader className="px-6 py-5 border-b border-slate-100 shrink-0 bg-white flex flex-row items-center justify-between space-y-0">
