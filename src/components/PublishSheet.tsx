...
export function PublishSheet({
    ...
    // Fetch products for active assignments in Manage view
    useEffect(() => {
    ...
    }, [view, currentFormAssignments.length]); // Optimized dependencies
        ...
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent className="sm:max-w-2xl w-full flex flex-col p-0 bg-slate-50">
                <SheetHeader className="px-6 py-4 bg-white border-b sticky top-0 z-10 flex flex-row items-center justify-between space-y-0">
                    <div className='space-y-1.5'>
                        <SheetTitle className="flex items-center gap-2">
                            ...
                            {view === 'manage' ? 'Active Deployments' : 'Publish Form'}
                        </SheetTitle>
                        <SheetDescription>
                            {view === 'manage'
                                ? "Manage where this form is currently active."
                                : "Choose a store to publish this form to."
                            }
                        </SheetDescription>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto flex flex-col min-h-0 bg-slate-50/50">
                    {view === 'publish' && (
                        <div className="flex-1 p-6 space-y-4 animate-in slide-in-from-right-4 duration-300">
                            {/* Step 1: Store Selection */}
                            {step === 1 && (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-sm font-semibold text-slate-900">Select Destination Store</h3>
                                    </div>
                    ...
                    {view === 'manage' && (
                        <div className="flex-1 overflow-y-auto px-6 py-4 bg-slate-50 focus:outline-none animate-in fade-in duration-300">
                            {currentFormAssignments.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-16 text-center">
                                    ...
                                    <Button variant="link" size="sm" onClick={() => setView('publish')} className="mt-4 text-indigo-600 font-semibold">
                                        Start Publishing
                                    </Button>
                                </div>
                            ) : (
