...
export const ShopifyEditor = () => {
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          ...
          <div className="text-center">
            <p className="text-[12px] font-bold text-slate-800 group-hover:text-blue-700">
              All Products
            </p>
        ...
        >
          ...
          <div className="text-center">
            <p className="text-[12px] font-bold text-slate-800 group-hover:text-indigo-700">
              Pick Products
            </p>
            <p className="text-[9px] text-slate-500 mt-0.5 font-medium">
              Specific items
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            {link.type === "store" ? (
              ...
              <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-1.5 py-px rounded-full">
                <Package size={8} /> {link.productIds?.length || 0} product
                {(link.productIds?.length || 0) !== 1 ? "s" : ""} linked
              </span>
            )}
          </div>
        </div>
  ...
  const StoreRow = ({
    ...
    <button
      ...
      onClick={(e) => {
        e.preventDefault();
        console.log("StoreRow clicked for store:", store.id);
        setPickingLinkType(store.id);
      }}
      className={`relative z-10 pointer-events-auto w-full flex items-center justify-between p-3 rounded-xl border ${
        dashed
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        >
          {dashed ? <Plus size={16} /> : <Store size={18} />}
        </div>
      ...
      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-200 font-bold text-[11px] shadow-sm hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-colors shrink-0 group-hover:bg-emerald-500 group-hover:text-white group-hover:border-emerald-500">
              ...
              {unlinkedStores.map((store) => (
                <div key={store.id} className="relative w-full block">
                  {pickingLinkType === store.id ? (
                    <LinkTypePicker
                      store={store}
                      onClose={() => setPickingLinkType(null)}
                    />
                  ) : (
