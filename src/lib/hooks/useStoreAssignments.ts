/**
 * useStoreAssignments — fetches form assignments from backend (single source of truth).
 *
 * Replaces useFormAssignments (Firebase) with data fetched from the backend
...
function getCached(key: string): BackendAssignment[] | null {
...
function setCache(key: string, data: BackendAssignment[]): void {
...
export function useStoreAssignments(stores: ConnectedStore[]) {
    ...
    const fetchAssignments = useCallback(async () => {
        ...
        try {
            const results: BackendAssignment[] = [];
            ...
            for (const store of connectedStores) {
                ...
                try {
                    ...
                    const typed = storeAssignments.map(a => ({
                        ...
                        // Use the Firebase store.id (not the backend UUID) so UI consumers
                    ...
                    console.warn(`[useStoreAssignments] Failed to fetch for ${subdomain}:`, (e as Error).message);
            ...
            if (fetchId === fetchIdRef.current) {
                setError((e as Error).message || 'Failed to fetch assignments');
            }
        } finally {
            if (fetchId === fetchIdRef.current) {
                setLoading(false);
