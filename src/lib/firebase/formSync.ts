...
/**
 * Propagates form configuration changes to all store assignments.
...
export async function propagateFormUpdate(userId: string, formId: string, formName: string, config: { addons?: Record<string, any>;[key: string]: unknown }) {
    try {
        console.log(`[Sync] Propagating updates for form ${formId}...`);
        ...
        if (assignmentsSnap.empty) {
            console.log(`[Sync] No active assignments for form ${formId}. Skipping sync.`);
            return;
        }

        console.log(`[Sync] Found ${assignmentsSnap.size} assignments to update.`);

        // 2. For each assignment, push the new config to the store
        ...
        console.log(`[Sync] Successfully synced form ${formId} to stores.`);
